...
for actual_img in inputs:
    ...
    area_cnts = filtrAreaDumb(raw_cnts, 200)
    ...
    for cnt in hw_area_cnts:
        ...
        # if (cnt[:, 0, 1] > 10).any() or (cnt[:, 0, 1] < w - 10).any():
        #     k_hw_area_cnts.append(cnt)
        # else:
        # break
    ...
    for cnt in k_hw_area_cnts:
        area = cv2.contourArea(cnt)
        obvod = cv2.arcLength(cnt, True)
        if area / obvod > 2.5:
            oo_k_hw_area_cnts.append(cnt)
    ...
    # zeleně ty, co byly ještě v velikosti
    # orig = cv2.drawContours(orig, area_cnts, -1, (0, 255, 0), 5)

    # modře ty, co byly po fitováním čtvercem dokola a šířka/výška
    orig = cv2.drawContours(orig, hw_area_cnts, -1, (255, 0, 0), 7)

    # červeně co zbyly po filtrováním krajů
    orig = cv2.drawContours(orig, k_hw_area_cnts, -1, (0, 0, 255), 9)

    # zeleně co zbyly po filtrováním krajů
    orig = cv2.drawContours(orig, oo_k_hw_area_cnts, -1, (0, 255, 0), 11)
